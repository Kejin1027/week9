...
module.exports = {
    ...
    addMovie: function (req, res) {
        Actor.findOne({ name: req.params.name1 }, function (err, actor) {
            if (err) return res.status(400).json(err);
            if (!actor) return res.status(404).json();
            Movie.findOne({ title: req.params.name2 }, function (err, movie) {
                if (err) return res.status(400).json(err);
    ...
    removeMovie: function(req,res){
        Actor.findOne({_id: req.params.id1}, function(err, actor){
            ...
            let movieID = toString(req.params.id2);
            for( i = 0; i < movies.length; i++){
                movies[i] = toString(movies[i])
                if(movies[i] === movieID){
                    movies.splice(i, 1)
                }
            }
